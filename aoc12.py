...
def find_paths(paths, node='start', path=[], count=0):
    ...
    for n in paths[node]:
        if n.islower() and n in path:
            continue
        count += find_paths(paths, n, path[:])
    return count


def part_two(paths):
    return find_paths_two(paths)


def find_paths_two(paths, node='start', path=[], count=0):
    ...
    for n in paths[node]:
        if not can_put_node(n, path):
            continue
        count += find_paths_two(paths, n, path[:])
    return count


def can_put_node(node, path):
